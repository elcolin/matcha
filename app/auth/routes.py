...
def _is_form_submit():
    """True when this looks like a plain HTML form post, not a fetch()/JSON client."""
    return request.get_json(silent=True) is None and bool(request.form)
...
def register():
    ...
    try:
        ...
        if is_form:
            ...
            return render_template("auth/register.html"), err.status
        raise
    ...
    if is_form:
        flash(
            f"Account created! Verification link (dev): "
...
def login():
    ...
    try:
        ...
        if is_form:
            flash(err.message, "error")
            return render_template("auth/login.html"), err.status
        raise
...
def request_password_reset():
    ...
    try:
        ...
        if is_form:
            flash(err.message, "error")
            return render_template("auth/password_reset_request.html"), err.status
        raise
    ...
    if is_form:
        flash(
            f"If your account exists, a reset email has been sent. "
            f"Reset link (dev): <a href='{reset_link}'>{reset_link}</a>",
            "success",
        )
        return redirect(url_for("auth.login"))
...
def confirm_password_reset(token):
    ...
    try:
        ...
        if is_form:
            flash(err.message, "error")
            return render_template("auth/password_reset_confirm.html", token=token), err.status
        raise
