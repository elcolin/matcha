...
def _allocate_ids(count: int):
    """Return a list of `count` ids to use, preferring unused gaps in `users.id`.
    If there aren't enough gaps, continue after the current max id.
    """
    ...
    while len(ids) < count:
        if cur not in existing:
            ids.append(cur)
        cur += 1
    # If we exhausted gaps but the highest existing id >= last allocated, ensure ids are unique
    max_existing = max(existing) if existing else 0
    if ids and ids[-1] <= max_existing:
        # replace tail with ids after max_existing
...
def main(count: int = 500, reuse_ids: bool = False):
    ...
    if reuse_ids:
        ...
        users = []
        for uid in ids:
            # generate_user currently doesn't accept explicit id; insert workaround here
            generated = None
            # create user by directly inserting with explicit id
